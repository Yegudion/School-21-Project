...
int main(int argc, char* argv[]) {
  ...
  if (argc > 1) {
    for (int i = 1; i < argc; i++) {
      if (argv[i][0] == '-') {  //проверяем флаг ли это
        if (!(i == argc - 1 && !(opened_file))) {
          continue;
        }
      } else {
        fd = open(argv[i], O_RDONLY);  //открытие файла
        opened_file++;
      }

      if (fd == -1) {  //вывод ошибки при отсутствии файла
...
void usage(char* program_name) {  //ошибка использования флагов
  printf("usage: %s [-eEnstTvb] [file ...]", program_name);

  exit(EXIT_FAILURE);
}

void cat(actions* a, int* fd, int* count_starts_of_line,
         int* start_of_line) {  //реализация сат
  ...
  while ((bytes_read = read(*fd, &ch, BUFFER_SIZE))) {
    ...
    if (a->number && *start_of_line) {  //считает строки
