...
START_TEST(s21_is_greater_or_equal_0) {
  s21_decimal src1 = s21_create(0, 9, 0, 0b10, 0);
  ...
  s21_decimal half_of_one = {{86, 0, 0, 0}};
  s21_set_dec_scale_factor(&half_of_one, 1);
  set_single_bit(&src1, 98, 1);

  // int sign1, sign2, factor_op_1, factor_op_2;
  // s21_get_dec_sign(src1, &sign1);
  // s21_get_dec_sign(half_of_one, &sign2);
  // s21_get_dec_scale_factor(src1, &factor_op_1);
  // s21_get_dec_scale_factor(half_of_one, &factor_op_2);
  ...
  // printf("\n %u\n", 0 - 1);
  // int res = s21_is_equal(src1, half_of_one);

  int res = s21_is_greater_or_equal(src1, half_of_one);
...
START_TEST(positive_zero_equal_negative_zero_e) {
  s21_decimal first = S21_DECIMAL_NULL;
  s21_decimal second = S21_DECIMAL_NULL;
  s21_set_dec_sign(&second, 1);  // Set second as negative zero
...
void srunner_add_compare_tests(SRunner *sr) {
  ...
  // is_greater_or_equal
  tcase_add_test(tc_compare, s21_is_greater_or_equal_0);
