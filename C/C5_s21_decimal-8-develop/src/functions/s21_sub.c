...
void s21_bit_sub(s21_decimal value_1, s21_decimal value_2,
                 s21_decimal *result) {
  int check = 0;
  int buff = 0;
  for (int i = 0; i < 96; i++) {
...
int s21_sub(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (!error) {
    ...
    if (value_1.bits[0] == 0 && value_1.bits[1] == 0 && value_1.bits[2] == 0) {
      s21_set_dec_sign(&value_2, !sign_2);
    ...
    if (swithc_flag && value_2.bits[0] == 0 && value_2.bits[1] == 0 &&
        value_2.bits[2] == 0) {
      s21_set_dec_sign(&value_1, !sign_1);
    ...
    if (sign_1 == 0 && sign_2 == 1) {  // 1 -> +; 2 -> -
      ...
      error = error || s21_add(value_1, value_2, result);
    } else if (sign_1 == 1 && sign_2 == 0) {  // 1 -> -; 2 -> +
      s21_set_dec_sign(&value_2, 1);
      error = error || s21_add(value_1, value_2, result);
