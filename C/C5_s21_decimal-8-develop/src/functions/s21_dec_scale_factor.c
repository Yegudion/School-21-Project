...
int s21_get_dec_scale_factor(
    s21_decimal value, int *result) {  //функция для получение степени числа
  int error = 1,
...
int s21_set_dec_scale_factor(
  ...
  if (value <= 28 && value >= 0) {
    ...
    s21_get_dec_scale_factor(buf, &check);
    if (check == value) {
      error = 0;
