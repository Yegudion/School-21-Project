...
void divide_by_10(s21_decimal* result, int shift);

int sub_multiply_by_10(s21_decimal* const result,
                       s21_decimal const* const buf_shift_1,
                       s21_decimal const* const buf_shift_3) {
...
void dec_multiplu_by_10(s21_decimal* result, int shift) {
  int scale_res;
  int overflow = 0;
  for (; shift && !overflow; shift -= 1) {
    s21_get_dec_scale_factor(*result, &scale_res);
    if (scale_res + 1 < 29) {
      int last_bit = 95;
      for (; (last_bit >= 0) && (!scan_single_bit(*result, last_bit));)
        last_bit -= 1;
      s21_decimal buf_shift_1 = *result, buf_shift_3 = *result;
      if (last_bit + 3 < 96) {
        ...
        scale_res += 1;
        s21_set_dec_scale_factor(result, scale_res);
      }
    } else
      overflow = 1;
  }
...
void cast_to_same_scale(s21_decimal* decimal1, s21_decimal* decimal2) {
  ...
  s21_get_dec_scale_factor(*decimal1, &scale_dec1);
  s21_get_dec_scale_factor(*decimal2, &scale_dec2);
  ...
  if (scale_dec1 != scale_dec2) {
    if (scale_dec1 < scale_dec2)
      ...
      s21_set_dec_scale_factor(decimal1, scale_dec2);
    }
  }
}
